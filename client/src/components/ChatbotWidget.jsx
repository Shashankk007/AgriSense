...
const ChatbotWidget = () => {
  ...
  useEffect(() => {
    if (isMenuOpen && user?.phoneNumber) {
      const fetchHistory = async () => {
        try {
          setIsLoadingHistory(true);
          const history = await getChatHistoryAPI(user.phoneNumber);
      ...
      setMessages([
        {
          id: 'greeting',
          sender: 'bot',
          text: `Hello ${user?.fullName || 'Farmer'}! How can AgriSense help you today?`,
        },
      ]);
    }
  }, [isOpen, messages.length, user]);

  // Do not render if the user is not authenticated (since we need mobile_number)
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const data = await sendChatMessageAPI(user.phoneNumber, userMessage.text, conversationId);
